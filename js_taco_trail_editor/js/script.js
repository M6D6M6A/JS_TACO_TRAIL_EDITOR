...
function unpack_trail(trail_file) {
    ...
    reader.onload = function() {
        ...
        for (i = 0; i < postionBuffer.byteLength; i += 12) {
            var x_f32value = float32BytesToFouble(postionBuffer.slice(i, i + 4));
            var y_f32value = float32BytesToFouble(postionBuffer.slice(i + 4, i + 8));
            var z_f32value = float32BytesToFouble(postionBuffer.slice(i + 8, i + 12));
...
function positionToHtml(i, x, y, z) {
    ...
    tableRow0.draggable = true;
    tableRow0.setAttribute("ondragstart", "dragStart(event)");
    tableRow0.setAttribute("ondragover", "dragOver(event)");
...
function float32BytesToFouble(ArrayBuffer) {
    var y_f32 = new Float32Array(ArrayBuffer);
